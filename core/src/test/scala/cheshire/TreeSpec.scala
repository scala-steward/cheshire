...
class TreeSpec extends Specification with Discipline with ScalaCheck with ScalacheckShapeless {
  ...
  checkAll("Apply[Tree]", CommutativeApplyTests[Tree].commutativeApply[Int, Int, Int])
  checkAll(
    "NonEmptyTraverse[Tree]",
    NonEmptyTraverseTests[Tree]
      .nonEmptyTraverse[Either[Unit, *], Int, Int, Int, Int, Option, Option])
  checkAll("Eq[Tree]", EqTests[Tree[Int]].eqv)
  checkAll("Hash[Tree]", HashTests[Tree[Int]].hash)
